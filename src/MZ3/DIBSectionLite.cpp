...
BOOL CDIBSectionLite::Load(BYTE* lpszMemory)
{
...
//    nBytes = file.Read(&BmpFileHdr, sizeof(BmpFileHdr));
	memcpy(&BmpFileHdr,&lpszMemory[nBytes],sizeof(BITMAPFILEHEADER));
...
//    nBytes = file.Read(&BmpInfo, sizeof(BITMAPINFOHEADER)); 
	memcpy(&BmpInfo,&lpszMemory[nBytes],sizeof(BITMAPINFOHEADER));
...
//      nBytes = file.Read(BmpInfo.ColorTable(), nColorTableSize);
		memcpy(BmpInfo.ColorTable(),&lpszMemory[nBytes],nColorTableSize);
		nBytes += nColorTableSize;
	}
	...
	long bitsPos = dwFileStart + BmpFileHdr.bfOffBits;
	
    // read the bits
//    nBytes = file.Read(pBits, nBitsSize);
	memcpy(pBits,&lpszMemory[nBytes],nBitsSize);
	nBytes += nBitsSize;
