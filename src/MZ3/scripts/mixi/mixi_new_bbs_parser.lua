...
type:set_info_type('category');								-- カテゴリ
type:set_service_type('mixi');								-- サービス種別
type:set_serialize_key('TOPIC');							-- シリアライズキー
type:set_short_title('トピック一覧');						-- 簡易タイトル
type:set_request_method('GET');								-- リクエストメソッド
-- url : list_bbs.pl?id=xxx
type:set_cache_file_pattern('mixi\\list_bbs_{urlparam:id}.html');	-- キャッシュファイル
type:set_request_encoding('euc-jp');						-- エンコーディング
...
function parsePostURL(data, html)
	...
	local action, enctype = html:match('<form [^>]-action="(add_.-)" enctype="(.-)".->');
	if action ~= nil then
		mz3.logger_debug(action);
		data:set_text('post_address', action);
	end
	
	if enctype ~= nil then
		-- @see constants.h
		if line_has_strings(enctype, 'multipart') then
